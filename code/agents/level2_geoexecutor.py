...
def load_before_geojson_for_plan(
    plan_path: Path,
    plan_suffix: str = "_plan_level_2.json",
) -> Optional[Dict[str, Any]]:
    """
    plan: city/bbox/model_id/{stem}_plan_level_2.json
...
- plan.parsed_from_instruction.target_id is null/empty
- plan.parsed_from_instruction.target_id is not a digits-only string
- plan.parsed_from_instruction.operation is null/empty or unsupported
- plan.parsed_from_instruction.distance_m is null/empty/non-numeric or <= 0

INPUTS YOU WILL RECEIVE
-----------------------
1) plan.json (intent-level), including:
   - parsed_from_instruction.target_id
   - parsed_from_instruction.operation (extend / shorten)
   - parsed_from_instruction.distance_m (REQUIRED, meters)
   - parsed_from_instruction.endpoint (optional; if absent, choose "tail" by default)
...
----------------------------------------------
- You MAY call get_feature_geometry(plan.parsed_from_instruction.target_id) for traceability.
...
3) set_length_change_m(delta_m=plan.parsed_from_instruction.distance_m)
...
----------------------------------------
- If plan.parsed_from_instruction.operation indicates "extend" / "expand" / "lengthen":
...
- Prefer default arguments for compute_length_change_m unless plan.json explicitly provides min/max/rule.
...
def iter_plan_paths(data_root: Path, suffix: str = "_plan_level_2.json"):
...
def main():
    ...
    parser.add_argument("--plan_suffix", type=str, default="_plan_level_2.json")
    ...
    print(f"[Info] Found {len(plan_files)} plan files (Level-2)")
    ...
    for idx, plan_path in enumerate(plan_files, start=1):
        ...
        try:
            ...
            # Case: target_id missing
            if not target_id:
                save_json(
                    out_dir / f"{stem}_fail_null_target_id_level_2.json",
                    {
            ...
            before_fc = None
            if args.execute:
                before_fc = load_before_geojson_for_plan(plan_path, plan_suffix=args.plan_suffix)
            ...
            for attempt in range(args.max_retries + 1):
                ...
                # save latest tool_calls
                save_json(out_dir / f"{stem}_tool_calls_level_2.json", tool_calls)
                ...
                if ok:
                    ...
                    # success => save after geojson
                    save_json(out_dir / f"{stem}_after_level_2.geojson", exec_result["state_fc"])
                    break
                else:
            ...
            # always save bundle
            save_json(out_dir / f"{stem}_attempts_bundle_level_2.json", attempts_bundle)

            if not sample_success:
                fail_count += 1
                save_json(
                    out_dir / f"{stem}_final_fail_level_2.json",
                    {
                        "plan_path": str(plan_path),
                        "max_retries": args.max_retries,
                        "bundle_file": str(out_dir / f"{stem}_attempts_bundle_level_2.json"),
                    },
                )
    ...
    summary_file = model_subdir / "level2_geoexecutor_running_time.txt"
